...
class Processor():
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            print("Using label path:", self.arg.train_feeder_args['label_path'])
    ...
    def eval(self, epoch, save_score=True, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            # 初始化置信度数组，形状为 (4599, 155)
            confidence_scores = np.zeros((4599, 155))
            score_index = 0  # 用于跟踪置信度数组的索引
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                with torch.no_grad():
                    ...
                    # 将输出转换为 softmax 置信度，并存储在置信度数组中
                    softmax_scores = torch.softmax(output, dim=1).detach().cpu().numpy()
                    confidence_scores[score_index:score_index + softmax_scores.shape[0]] = softmax_scores
                    score_index += softmax_scores.shape[0]
        ...
        # 保存置信度文件
        np.save('pred.npy', confidence_scores)
...
if __name__ == '__main__':
    ...
    # 设置最佳权重文件路径
    #arg.weights = 'runs/2202-42-12695.pt'  # 替换为最佳权重文件的路径
