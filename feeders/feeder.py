...
#         return data_numpy, label, index
    ...
    def __getitem__(self, index):
        ...
        # 打印标签信息
        #print(f"Label: {label}, Type: {type(label)}, Shape: {np.shape(label)}")

        # 处理标签
        if isinstance(label, str):
            label_tensor = torch.tensor(int(label), dtype=torch.long)
        elif isinstance(label, (list, np.ndarray)):
            label_tensor = torch.tensor(label, dtype=torch.long).flatten()  # 确保一维
        else:
            label_tensor = torch.tensor(label, dtype=torch.long)

        return data_numpy, label_tensor, index
...
if __name__ == '__main__':
    ...
    label_path = "../data/ntu/xview/val_label.npy"
